...
from Path_Planning import generate_corners, generate_path
...
def build_gui():
    ...
    default_gps = [( 40.443272, -79.958662), (40.443235, -79.958610),
                   (40.443517, -79.958234),(40.443529, -79.958298)]
    main_window = tk.Tk()
    ...
    main_window.start_button = ttk.Button(control_frame, text="Start", style="TButton",
                                          command=lambda: asyncio.create_task(BLE.start_task()))
    main_window.start_button.grid(row=0, column=0, padx=20, pady=10, sticky="nsew", ipadx=20)

    main_window.stop_button = ttk.Button(control_frame, text="Stop", style="TButton",
    ...
    main_window.pause_button = ttk.Button(control_frame, text="Pause", style="TButton",
    ...
    main_window.bt_button = ttk.Button(bluetooth_frame, text="Connect Bluetooth",
                                       command=lambda: asyncio.create_task(BLE.connect()))
    ...
    main_window.markers = [
        ...
        main_window.map_widget.set_marker(37.6879, -122.4702, text="Corner 3")
    ]
    ...
    style = ttk.Style()
    style.configure("Red.TButton", foreground="red", font=("Arial", 25), padding=0)
    main_window.estop_button = ttk.Button(
        main_window,
        text="ESTOP",
        style="Red.TButton",
        command=lambda: asyncio.create_task(BLE.write("STOP\t"))
    )
    main_window.estop_button.pack()
...
def gps_update():
    for i, (lat_entry, lon_entry) in enumerate(main_window.gps_fields):
        try:
            ...
            if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
                messagebox.showerror("Invalid latitude or longitude", message="Entries for Marker" + str(
                    i + 1) + "are not valid points, please try again.")
                return

            if len(lat_entry.get().split('.')[-1]) < 4 or len(lon_entry.get().split('.')[-1]) < 4:
                messagebox.showerror("Not Enough Precision",
                                     "Coordinates for Marker" + str(i + 1) + " must have at least 4 decimal places.")
                return
    ...
    try:
        ...
        corners = generate_corners(gps_points[0],gps_points[1],gps_points[2],gps_points[3])
        ...
        if test_path is not None:
            ...
            messagebox.showerror("Non Viable Points","The given points cannot generate a viable polygon for seraching")
            return
...
class BLE():
    ...
    async def start_task(self):
        ...
        await self.write("C4:" + str(corners[3][0]) + "," + str(corners[3][1]) + "\n")
    ...
    async def read_callback(self, sender: bleak.BleakGATTCharacteristic, data: bytearray):
        ...
        if data[:4] == "WIRE":
            ...
            pickup = messagebox.askyesno("Trash Detected at : ", message=data[6:])
            if pickup == True:
