...
global YOLO_home,PY_home
YOLO_home = (os.getcwd()+'/Main/Ros2 Main/Py_Modules/YOLOv8/').replace('\\','/')
PY_home = (os.getcwd()+'/Main/Ros2 Main/Py_Modules/').replace('\\','/')
print(f"YOLO_home:\t\t<{YOLO_home}>")
print(f"PY_home:\t\t<{PY_home}>")
...
if __name__ == "__main__":
    ...
    #running
    start_time = time.time()
    result = test_model.run_model(PY_home+'Testing/TEST_example.jpg')
    end_time = time.time()
    ...
    #running square
    start_time = time.time()
    result = test_model.run_model(PY_home+'Testing/TEST_example_square.jpg')
    end_time = time.time()
    ...
    #saving
    start_time = time.time()
    result = test_model.save_model(PY_home+'Testing/TEST_initModelN.onnx')
    end_time = time.time()
    ...
    #saving
    start_time = time.time()
    result = test_model.save_model(PY_home+'Testing/TEST_initModelS.onnx')
    end_time = time.time()
    ...
    test_model = YOLO_model_v1(model_path=PY_home+'Testing/example_model.pt')
    ...
    #saving
    start_time = time.time()
    result = test_model.save_model(PY_home+'Testing/TEST_LoadModPT.onnx',imgsz=[512,384])
    end_time = time.time()
    ...
    test_model = YOLO_model_v1(model_path=PY_home+'Testing/TEST_LoadModPT.onnx')
    ...
    #saving (not allowed)
    try:
        result = test_model.save_model(PY_home+'Testing/TEST_LoadModONNX.onnx')
    ...
    test_model = YOLO_model_v1(model_path=PY_home+'Testing')
    #demo of the compression
    result = test_model.run_model(PY_home+'Testing/TEST_example.jpg')
    print(Back.BLUE+f'model5 Run output:\t{result}'+Style.RESET_ALL)
    for i,res in enumerate(result):
        reduce_filepath(
            file_path=  PY_home+'Testing/TEST_example.jpg',
            coords=     res[1],
            output_path=    PY_home+f'Testing/Reduce_{i}__{res[0]}.jpg',
            Expan_rate=0.2
        )
